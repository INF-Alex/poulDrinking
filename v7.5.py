...
# if not os.path.exists(now_dir+'logs'):
#     os.makedirs(now_dir+'logs')
# # sys.stdout = Logger(now_dir+'logs/out.txt', sys.stdout)
# # sys.stderr = Logger(now_dir+'logs/warning.txt', sys.stderr)
...
def main(p_dir='p1.jpg',X=7,Y=7):
    ...
    for p_dir in jpg_files:
        ...
        suffix = "_" + str(p_dir.split('/')[-1].split('.')[0])
        filename = "result"+str(suffix)+".txt"
